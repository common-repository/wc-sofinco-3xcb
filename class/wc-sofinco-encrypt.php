<?php
/**
 * Sofinco WooCommerce Module
 *
 * Feel free to contact Sofinco at support@sofinco.fr for any
 * question.
 *
 ...
 * support@sofinco.fr so we can mail you a copy immediately.
 *
 * @author Guillaume - BM Services (http://www.bm-services.com)
 * @copyright 2012-2015 Sofinco
 * @license http://opensource.org/licenses/OSL-3.0
 * @link http://www.sofinco.com/
 * @since 2
 * */
...
class SofincoEncrypt
{
    ...
    public function generateKey()
    {
        ...
        if (file_exists(WC_SOFINCO_KEY_PATH)) {
            unlink(WC_SOFINCO_KEY_PATH);
        }
        $key = bin2hex($key);
        $iv = $this->_MakeIv($key);
        file_put_contents(WC_SOFINCO_KEY_PATH, "<?php" . $key.$iv ."?>");
        // Init vector
    }
    /**
    ...
    private function _getKey()
    {
        ...
        if (!file_exists(WC_SOFINCO_KEY_PATH)) {
            $this->generateKey();
            $_POST['KEY_ERROR'] = __("For some reason, the key has just been generated. please reenter the HMAC key to crypt it.", WC_SOFINCO_PLUGIN);
        } else {
            $key = file_get_contents(WC_SOFINCO_KEY_PATH);
            $key = substr($key, 5, 32);
            return $key;
        }
    }
    ...
    private function _getIv()
    {
        ...
        if (!file_exists(WC_SOFINCO_KEY_PATH)) {
            ...
            $iv = file_get_contents(WC_SOFINCO_KEY_PATH);
            $iv = substr($iv, 37, 16);
            return $iv;
        }
    }
    ...
    private function _decrypt($key, $iv, $data)
    {
        if (function_exists('openssl_decrypt')) {
            ...
            if (!$result) {
                show_message(new WP_Error("", "", "ATTENTION:le module ne peut plus afficher la clef HMAC, veuillez l'initialiser de nouveau."));
            }
        } else {
            // Prepare mcrypt
            $td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
     ...
     * @see SOFINCO_Epayment_Helper_Encrypt::_getKey()
     */
